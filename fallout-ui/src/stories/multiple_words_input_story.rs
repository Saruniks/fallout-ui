...
pub fn MultipleWordsInputStory() -> Html {
    let (form_handle, fields_control_props) = form_schema::use_form_schema(|| {
        form_schema::FormSchemaHandle::new(form_schema::FormSchema::default())
    });
    let (form_handle_2, fields_control_props_2) = form_schema_2::use_form_schema_2(|| {
        form_schema_2::FormSchemaHandle::new(form_schema_2::FormSchema2::default())
    });

    use_effect_with((), move |_| {
        form_handle_2.dispatch(form_schema_2::FormSchema2Action::TouchAll);
        || {}
    });

    let on_submit = move |_| {
        form_handle.dispatch(form_schema::FormSchemaAction::TouchAll);
