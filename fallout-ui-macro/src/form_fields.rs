...
pub fn form_fields_impl(input: DeriveInput) -> Result<proc_macro2::TokenStream> {
    ...
    Ok(quote! {
                ...
                {
                    pub values: S,
                    pub errors: std::collections::HashMap<F, String>,
                    pub touched_fields: std::collections::HashSet<F>,
                }

                impl<S, F> FormSchemaHandle<S, F>
                where
                    S: validator::Validate + Clone + PartialEq,
                    F: Eq + std::hash::Hash + std::str::FromStr + strum::IntoEnumIterator,
                {
                    pub fn new(initial_values: S) -> Self {
                        let mut form_schema_handle = Self {
                            values: initial_values,
                            errors: std::collections::HashMap::<F, String>::default(),
                            touched_fields: std::collections::HashSet::<F>::default(),
                        };
